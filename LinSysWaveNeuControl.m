...
%
% on [0,1] with Neumann boundary conditions w_\xi(0,t)=w_\xi(1,t)=0. 
...
w_0 = @(xi) cos(pi*xi)+3*cos(5*pi*xi);
...
% boundary conditions
phi_n = @(xi,n) sqrt(2)*cos(n*pi*xi);
phi_n_prime = @(xi,n) sqrt(2)*n*pi*sin(n*pi*xi);
...
A_0 = spdiags(lambda_n((0:(N-1)).'),0,N,N);

B_0 = zeros(N,1);
C_1 = zeros(1,N);
C_2 = zeros(1,N);
for ind = 1:N
  B_0(ind) = phiprod(b,ind-1);
  C_1(ind) = phiprod_prime(c_1,ind-1);
  C_2(ind) = phiprod(c_2,ind-1);
end

D_0 = zeros(N,N);
for ind1 = 1:N
  for ind2 = ind1:N
    D_0(ind1,ind2) = phiprod(@(xi) d(xi).*phi_n(xi,ind2-1),ind1-1);
  end
...
% evals = eig(full(AN-1.4*(CN')*BN'));
evals = eig(full(AN-BN*CN));
plot(real(evals),imag(evals),'b.','Markersize',20)
axis([-.5,0,-4,4])
%%
...
w1_vec = zeros(N,1);
for ind = 1:N
  w0_vec(ind) = phiprod(w_0,ind-1);
  w1_vec(ind) = phiprod(w_1,ind-1);
end
